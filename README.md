## 📋 Overview

This is my personal portfolio website built as **Task 1** for the **Future Interns Full Stack Web Development Internship**. It showcases my skills, projects, education, and professional journey as a Computer Science student at Dire Dawa University.

## 🚀 Live Demo
🔗 https://Ace0407-akliseya.github.io/FUTURE_FS_01/

## 🛠️ Built With
- HTML5, CSS3, JavaScript
- Font Awesome, Google Fonts
- Git & GitHub Pages

## ✨ Key Features
✅ Fully responsive design  
✅ Modern teal/charcoal theme  
✅ Smooth scrolling & hover effects  
✅ Project showcase with tech stacks  
✅ Contact cards & form  

## 📂 Sections
- Hero | About | Education | Skills | Projects | Contact
## 📁 Project Structure
FUTURE_FS_01/
│
├── index.html # Main HTML file
├── style.css # All styles
├── script.js # JavaScript functionality
├── README.md # Project documentation
├── image/
   ├── profile.jpg

## 📁 Projects
| Project | Tech |
|---------|------|
| Scientific Calculator | Java, Swing |
| ATM Simulation | Java, File I/O |
| Food Delivery Website | HTML, CSS, JS |
| Contact Book | C++, STL |

## 👩‍💻 Author
**Akliseya Shiferaw**  
📧 akliseya.shiferaw@example.com  
🔗 [LinkedIn](https://www.linkedin.com/in/akliseya-shiferaw-85b8aa39b)  
🔗 [GitHub](https://github.com/Ace0407-akliseya)

## ✅ Task Requirements
- [x] Interactive resume sections
- [x] Responsive design
- [x] Contact form
- [x] SEO friendly
- [x] GitHub repo: `FUTURE_FS_01`

---

*Future Interns Full Stack Development – Task 1*